...
// Variant ID for the gift box product in Medusa.
// Set NEXT_PUBLIC_GIFT_BOX_VARIANT_ID in .env.local to enable add-to-cart.
const GIFT_BOX_VARIANT_ID = process.env.NEXT_PUBLIC_GIFT_BOX_VARIANT_ID || ""
...
const UpsellPopup = ({
  ...
  const [giftBoxAdding, setGiftBoxAdding] = useState(false)
  ...
  useEffect(() => {
    if (show) {
      ...
      setIsAnimated(false)
      // Reset gift box state when popup closes so it resets for next open
      setGiftBoxAdded(false)
    }
  }, [show])

  const handleAddGiftBox = async () => {
    if (!GIFT_BOX_VARIANT_ID || giftBoxAdding || giftBoxAdded) return

    setGiftBoxAdding(true)
    try {
      await addToCart({
        variantId: GIFT_BOX_VARIANT_ID,
        quantity: 1,
        countryCode,
      })
        ...
        <div className="p-6 small:p-8">
          ...
          <div className="border-t border-grey-10 pt-5 mb-1">
            <button
              ...
              disabled={giftBoxAdding || giftBoxAdded || !GIFT_BOX_VARIANT_ID}
              aria-label="Добави подаръчна кутия за 2.99 EUR"
              className={clx(
                "w-full flex items-center gap-4 p-4 border rounded-xl transition-colors text-left",
                "focus:outline-none focus-visible:ring-2 focus-visible:ring-grey-40",
                giftBoxAdded
                  ? "border-emerald-200 bg-emerald-50/50 cursor-default"
                  : !GIFT_BOX_VARIANT_ID
                    ? "border-grey-10 bg-grey-5 opacity-60 cursor-not-allowed"
                    : "border-gold-100 bg-gold-50/30 hover:bg-gold-50/50 cursor-pointer group/gift"
              )}
            >
              ...
              <div className="text-right flex-shrink-0">
                <p className="font-sans text-sm font-medium text-grey-90">+2.99 EUR</p>
                <span
                  className={clx(
                    "font-sans text-[0.65rem] font-medium tracking-wide uppercase mt-0.5",
                    giftBoxAdded
                      ? "text-emerald-500"
                      : "text-soft-gold group-hover/gift:underline"
                  )}
                >
                  {giftBoxAdding ? "..." : giftBoxAdded ? "Добавено" : "Добави"}
                </span>
              </div>
            </button>
          </div>
